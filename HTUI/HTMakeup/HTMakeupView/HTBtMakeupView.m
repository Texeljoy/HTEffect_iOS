...
#import "HTMakeupEffectViewCell.h"
...
@interface HTBtMakeupView ()<UICollectionViewDataSource, UICollectionViewDelegate>
...
- (instancetype)initWithFrame:(CGRect)frame listArr:(NSArray *)listArr{
    ...
    if (self) {
        ...
        // 颜色切换的通知
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(switchColor:) name:@"HTMakeupColorSwitchNotification" object:nil];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (collectionView.tag == MakeupLevel_First) {
        HTMakeupEffectViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"HTMakeupEffectViewCell" forIndexPath:indexPath];
...
//        NSLog(@"55555555555555555 = %@", self.selectedModel.name);
    ...
    if (indexPath.row == 0) {
        ...
        if (self.selectedModel.idCard == 0 || self.selectedModel.idCard == 1 || self.selectedModel.idCard == 2) {
            [[HTEffect shareInstance] setMakeup:self.selectedModel.idCard property:@"type" value:self.selectedModel.type];
            [[HTEffect shareInstance] setMakeup:self.selectedModel.idCard property:@"color" value:@""];
            [[HTEffect shareInstance] setMakeup:self.selectedModel.idCard property:@"value" value:@"0"];
        }else {
            [[HTEffect shareInstance] setMakeup:self.selectedModel.idCard property:@"name" value:self.selectedModel.name];
            [[HTEffect shareInstance] setMakeup:self.selectedModel.idCard property:@"value" value:@"0"];
        }
...
//            self.selectedModel = indexModel;
        ...
        if (self.selectedModel.idCard == 0 || self.selectedModel.idCard == 1 || self.selectedModel.idCard == 2) {
            [[HTEffect shareInstance] setMakeup:self.selectedModel.idCard property:@"type" value:self.selectedModel.type];
            [[HTEffect shareInstance] setMakeup:self.selectedModel.idCard property:@"color" value:self.currentColorArray[self.selectedModel.idCard]];
            [[HTEffect shareInstance] setMakeup:self.makeupSelectedModel.idCard property:@"value" value:[NSString stringWithFormat:@"%d", (int)selectedValue]];
        }else {
            [[HTEffect shareInstance] setMakeup:self.makeupSelectedModel.idCard property:@"name" value:self.selectedModel.name];
            [[HTEffect shareInstance] setMakeup:self.makeupSelectedModel.idCard property:@"value" value:[NSString stringWithFormat:@"%d", (int)selectedValue]];
        }
    }
...
- (void)restore{
    ...
    for (int i = 0; i < self.makeupArray.count; i++) {
        ...
        if (model.idCard == 0 || model.idCard == 1 || model.idCard == 2) {
            [[HTEffect shareInstance] setMakeup:model.idCard property:@"type" value:model.type];
            [[HTEffect shareInstance] setMakeup:model.idCard property:@"color" value:@""];
            [[HTEffect shareInstance] setMakeup:model.idCard property:@"value" value:@"0"];
        }else {
            [[HTEffect shareInstance] setMakeup:model.idCard property:@"name" value:model.name];
            [[HTEffect shareInstance] setMakeup:model.idCard property:@"value" value:@"0"];
        }
...
- (void)restoreEffect {
    ...
    for (NSInteger i = 0; i < self.detailModelArray.count; i++) {
        ...
        if (model.idCard == 0 || model.idCard == 1 || model.idCard == 2) {
            [[HTEffect shareInstance] setMakeup:model.idCard property:@"type" value:model.type];
            [[HTEffect shareInstance] setMakeup:model.idCard property:@"color" value:self.currentColorArray[model.idCard]];
            [[HTEffect shareInstance] setMakeup:model.idCard property:@"value" value:[NSString stringWithFormat:@"%d", (int)[HTTool getFloatValueForKey:model.key]]];
        }else {
            [[HTEffect shareInstance] setMakeup:model.idCard property:@"name" value:model.name];
            [[HTEffect shareInstance] setMakeup:model.idCard property:@"value" value:[NSString stringWithFormat:@"%d", (int)[HTTool getFloatValueForKey:model.key]]];
        }
...
-(void)switchColor:(NSNotification *)notification{
    ...
    [[HTEffect shareInstance] setMakeup:self.selectedModel.idCard property:@"type" value:self.selectedModel.type];
    [[HTEffect shareInstance] setMakeup:self.selectedModel.idCard property:@"color" value:dict[@"color"]];
    [[HTEffect shareInstance] setMakeup:self.makeupSelectedModel.idCard property:@"value" value:[NSString stringWithFormat:@"%d", (int)[HTTool getFloatValueForKey:self.selectedModel.key]]];
}
...
- (void)updateEffectWithValue:(int)value {
    ...
    if (self.selectedModel.idCard == 0 || self.selectedModel.idCard == 1 || self.selectedModel.idCard == 2) {
        [[HTEffect shareInstance] setMakeup:self.selectedModel.idCard property:@"type" value:self.selectedModel.type];
        [[HTEffect shareInstance] setMakeup:self.selectedModel.idCard property:@"color" value:self.currentColorArray[self.selectedModel.idCard]];
        [[HTEffect shareInstance] setMakeup:self.makeupSelectedModel.idCard property:@"value" value:[NSString stringWithFormat:@"%d", value]];
    }else {
        [[HTEffect shareInstance] setMakeup:self.makeupSelectedModel.idCard property:@"name" value:self.selectedModel.name];
        [[HTEffect shareInstance] setMakeup:self.makeupSelectedModel.idCard property:@"value" value:[NSString stringWithFormat:@"%d", value]];
    }
...
//        _editCollectionView.alwaysBounceHorizontal = YES;
        [_makeupCollectionView registerClass:[HTMakeupEffectViewCell class] forCellWithReuseIdentifier:@"HTMakeupEffectViewCell"];
...
- (void)dealloc{
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:@"HTMakeupColorSwitchNotification"];
}
