...
- (void)downloadSuccessedType:(DownloadedType)type htModel:(HTModel *)model completeBlock:(void(^)(BOOL successful))completeBlock{
    ...
    switch (type) {
        case HT_DOWNLOAD_TYPE_Sticker:// 贴纸
            downloadURL = [[HTEffect shareInstance].getStickerUrl stringByAppendingFormat:@"%@.zip",model.name];
            cachePaths =  [HTEffect shareInstance].getStickerPath;
            break;
        case HT_DOWNLOAD_STATE_Greenscreen:// 绿幕抠图
            downloadURL = [[HTEffect shareInstance].getGSSegEffectUrl stringByAppendingFormat:@"%@.png",model.name];
            cachePaths =  [HTEffect shareInstance].getGSSegEffectPath;
            break;
        case HT_DOWNLOAD_STATE_Portraits:// AI抠图
